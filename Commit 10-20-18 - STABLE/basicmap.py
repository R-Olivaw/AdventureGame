# -*- coding: utf-8 -*-
"""
Created on Wed Oct 17 21:24:39 2018
...
rusty_key = items.Door_Key("Rusty Key", "A really rusty key", 0, 666)
...
find_weapon_room = world.FindItemTile(0, 1, silver_sword, utilities.boxer.make_text_box("You find a gleaming silver sword!"), utilities.boxer.make_text_box("An empty rooom in which the faint smell of glory lingers in the air."))
...
cow_room = world.SingleEnemyTile(0, 2, crazed_cow, utilities.boxer.make_text_box("A wild cow raises its hoof in anger!"), utilities.boxer.make_text_box("A cow corpse rots on the floor."))
...
find_key_room = world.FindItemTile(0, 3, rusty_key, "A room with a key in it!", "A room with no key in it!")

first_passage = world.SimplePassage(0, 4, "An empty room with a door at the far side.")

devil_door = world.LockedRoom(0, 5, False, "I'm a locked door, oh no!", "I'm an unlocked door, yeah!", 666)

second_passage = world.SimplePassage(0, 6, "An empty room with a door at the near side.")
...
game_map[(0,1)] = find_weapon_room
game_map[(0,2)] = cow_room
game_map[(0,3)] = find_key_room
game_map[(0,4)] = first_passage
game_map[(0,5)] = devil_door
game_map[(0,6)] = second_passage
game_map[(0,7)] = end_room
